...
/**
 * Rate limiter for public leads endpoint
 * Uses settings.security.rateLimits.publicRPM or defaults to 5 requests per minute, 50 requests per day per IP + phone
 */
@Injectable()
export class LeadRateLimitMiddleware implements NestMiddleware {
  ...
  private async getMaxRequestsPerMin(): Promise<number> {
    ...
    return settings.security?.rateLimits?.publicRPM ?? 5;
  }
  ...
  private async getMaxRequestsPerDay(): Promise<number> {
    ...
    return perMin * 10; // Default: 50 if perMin is 5
  }

  async use(req: Request, res: Response, next: NextFunction) {
    // Only apply to POST /v1/public/leads/sell
    if (req.method !== 'POST' || !req.path.includes('/public/leads/sell')) {
      return next();
    }

    const tenantId = (req as any).tenantId || 'et-addis';
    const phone = (req.body as any)?.phone || '';
    const clientId = this.getClientId(req, tenantId, phone);
    const now = Date.now();
  ...
  private getClientId(req: Request, tenantId: string, phone: string): string {
    // Use IP address + phone + tenant ID for rate limiting
    ...
    return `lead:${tenantId}:${ip}:${normalizedPhone}`;
  }
