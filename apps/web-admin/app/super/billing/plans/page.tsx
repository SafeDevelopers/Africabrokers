...
export default function AgencyPlansPage() {
  ...
  const loadPlans = async () => {
    try {
      setLoading(true);
      // Note: This endpoint would need to be created to list all agency plans
      // For now, we'll use a placeholder
      const data = await apiClient.get("/superadmin/billing/plans", {
        includeTenant: false,
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPlan) {
        // Update plan (endpoint would need to be added)
        await apiClient.put(`/superadmin/billing/plans/${editingPlan.id}`, formData, {
        includeTenant: false,
      });
      } else {
        await apiClient.post("/superadmin/billing/plans", formData, {
        includeTenant: false,
      });
      }
  ...
  const handleDelete = async (planId: string) => {
    ...
    try {
      await apiClient.delete(`/superadmin/billing/plans/${planId}`, {
        includeTenant: false,
      });
      loadPlans();
    } catch (err) {
  ...
  const handleEdit = (plan: Plan) => {
    ...
    setFormData({
      ...
      audience: (plan.metadata as any)?.audience || "tenant",
      planId: plan.planId,
      name: plan.name,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Agency Plans</h1>
          <p className="mt-2 text-gray-600">Manage subscription plans for agencies/tenants</p>
        </div>
        <button
          onClick={() => {
            setEditingPlan(null);
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900">{plan.tenant?.name || plan.tenantId}</span>
                </div>
                <div className="flex justify-between text-sm">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">
                {editingPlan ? "Edit Plan" : "Add Agency Plan"}
              </h2>
