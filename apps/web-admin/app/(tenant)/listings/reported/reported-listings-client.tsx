...
type ReportedListingsClientProps = {
  initialListings: ReportedListing[];
};

export function ReportedListingsClient({ initialListings }: ReportedListingsClientProps) {
  const [listings] = useState<ReportedListing[]>(initialListings);
  ...
  }, [listings, statusFilter, severityFilter]);
...
function ReportedListingRow({ listing }: { listing: ReportedListing }) {
  ...
  const handleReportAction = (action: string) => {
    ...
    alert(`${action} report for: ${listing.title} (API call would be made here)`);
  };
